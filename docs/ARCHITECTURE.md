...
15. **RSSHub is a stabilizer, not a rescue — but you can write the missing routes** — a self-hosted RSSHub was deployed; measured reality: of 61 sources only OpenAI + Anthropic had live community routes, and the official Docker image ships only the prod build (`dist/`, `lib/` deleted) so its routes are compile-time only. The correction: RSSHub is **not** limited to existing routes — the source tree in **dev mode** dynamically loads `lib/routes/` (drop a file, tsx watch reloads, no rebuild), and `registerRoute` is the npm package's main entry. We wrote custom routes for the gaps: `/cnsa/news` (P3 China-space official, the biggest hole) and `/esa/newsroom`. Key distinction: the **Docker image** is the server build (routes baked in); the **source-tree dev mode** is the extensible build (routes live). The 3 truly anti-bot-blocked sources (WaPo/MS/Blue Origin) still can't be fixed by any RSSHub route — those need alternative sources, not a route.
16. **Community routes rot — audit, don't assume** — of the community routes pointing at our sources, 10 were dead (TASS 404, NASA/Nature/MS/GitHub/SCMP/ChinaDaily/CFR/Yahoo empty). A route breaks whenever the origin site changes and nobody re-maintains it. The fix pattern: `xxxfix/news.ts` (got + browser UA + `responseType:'text'`, or ofetch + `parseResponse: t=>t`), verified live, mapped in sources.json. 10 fix routes all work (chinadailyfix 100, naturefix 75, cfrfix 72, tassfix 33).
17. **Anti-bot is client-specific** — SpaceNews 403 / New Scientist 406 reject the RSSHub client (got/ofetch UA fingerprint) but accept Python direct fetch. Don't fight it: keep such sources on direct fetch (their `mode: rss` already works). "RSSHub can't fix it" is only true per-client, not per-site.
18. **New top-level route dir needs a restart; file edits don't** — `registry-dev.ts` `readdirSync` scans top-level dirs once at startup; a newly added `lib/routes/<newdir>/` isn't picked up until the dev server restarts, while editing a file inside an existing dir auto-reloads via tsx watch.
