...
15. **RSSHub is a stabilizer, not a rescue** — a self-hosted RSSHub (Docker, persistent) was deployed on the theory that direct scraping was the problem. Measured reality: of 61 sources, only OpenAI + Anthropic have live RSSHub news routes (TASS/Nature routes dead, NASA/CGTN/MS/DeepSeek non-news, ~20 others no route at all). The 3 truly failing sources (WaPo/MS/Blue Origin) are anti-bot blocks RSSHub can't bypass. Wire the routes that exist (`rsshub` field → prefer route, fall back to direct scrape) and keep direct scraping as the backbone — never let a community-maintained feed become a single point of failure.
